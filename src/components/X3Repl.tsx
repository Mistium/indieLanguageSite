...
export default function X3Repl() {
  const editorRef = useRef<HTMLDivElement>(null);
  const [editor, setEditor] = useState<CodeMirrorEditor | null>(null);
  const [editorReady, setEditorReady] = useState(false);
  const [files, setFiles] = useState<Record<string, string>>({ "main.x3": "" });
  const [activeFile, setActiveFile] = useState("main.x3");
  ...
  useEffect(() => {
    ...
    const instance = CodeMirror(editorRef.current, {
      mode: "x3", // Adjust if you later define a custom X3 mode
      lineNumbers: true,
      theme: "material",
    });
  ...
  const addFile = () => {
    const name = prompt("Enter file name (will end in .x3):");
    if (!name) return;
    const safeName = name.endsWith(".x3") ? name : `${name}.x3`;
  ...
  const runCode = async () => {
    ...
    setOutput("Running...");
    try {
      const res = await fetch("https://indielangsbackend.onrender.com/api/run/X3", {
        method: "POST",
    ...
    <div className="repl-container">
      ...
      <div className="repl-editor-panel">
        <div className="repl-tabs">
          ...
          <button onClick={addFile} className="repl-add-tab">+</button>
        </div>
